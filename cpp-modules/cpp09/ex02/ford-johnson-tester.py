...
def format_result(label: str, result: int | float, okcolor: str, maximal_number_of_comparisons: int):
...
def run_test(executable_path: pathlib.Path, test_input: list[str], output_check: bool, maximal_number_of_comparisons: int):
    ...
    number_of_comparisons_match = re.search(r"^(OK|KO): (\d+) comparisons \(max for n = (\d+) is (\d+)\)", cleaned_ouput, re.MULTILINE)
    if not number_of_comparisons_match:
        return ErrorCode.NO_NUMBER_OF_COMPARISONS_IN_OUTPUT, cleaned_ouput

    number_of_comparisons = int(number_of_comparisons_match.group(2))
...
if __name__ == "__main__":
    ...
    for i, test_range in enumerate(ranges_to_test):
        ...
        with ProcessPoolExecutor() as executor:
            futures = [
                executor.submit(run_test, args.executable, test_input, args.output_check, maximal_number_of_comparisons) for test_input in test_inputs
            ]
        ...
        worst_result_str = format_result("Worst result:                ", worst_result, C.OKGREEN, maximal_number_of_comparisons)

        best_result = min(results)
        best_result_str = format_result("Best result:                 ", best_result, C.OKBLUE, maximal_number_of_comparisons)

        average_result = sum(results) / len(results)
        average_result_str = format_result("Average result:              ", average_result, C.OKCYAN, maximal_number_of_comparisons)
